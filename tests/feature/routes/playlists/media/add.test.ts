...
describe("The playlist medium add route", () => {
  ...
  it("can filter available media by title or description", async () => {
    ...
    await supertest(express)
      .get(`/playlists/${playlist.id}/media/add?search=${medium1.title}`)
      .expect(200)
      .expect((res) => {
    ...
    await supertest(express)
      .get(`/playlists/${playlist.id}/media/add?search=baz`)
      .expect(200)
      .expect((res) => {
  ...
  it("can sort the list based on other columns in ascending or descending order", async () => {
    ...
    await supertest(express)
      .get(`/playlists/${playlist.id}/media/add?sort=title`)
      .expect((res) => {
    ...
    await supertest(express)
      .get(`/playlists/${playlist.id}/media/add?sort=title&sortBy=asc`)
      .expect((res) => {
